...
import com.profitz.app.RichEditor.RichEditor;
import com.profitz.app.WysiwygUtils.ImgPicker;
import com.profitz.app.WysiwygUtils.Keyboard;
import com.profitz.app.WysiwygUtils.TextColor;
import com.profitz.app.WysiwygUtils.Youtube;
import com.profitz.app.WriteCustomButton;
...
public class RichWysiwyg extends LinearLayout {
    ...
    private WriteCustomButton textBullets;
    private WriteCustomButton textNumbers;
    ...
    class DecorationButtonListener implements OnClickListener{
        ...
        public void onClick(View view){
            if(view instanceof WriteCustomButton) {
                ...
                else if(button.getId() == R.id.write_textNumbers)
                    content.setNumbers();
                else if(button.getId() == R.id.write_textBullets)
                    content.setBullets();
                if(button.getCheckedState()) {
                    ...
                    button.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                    button.switchCheckedState();
                }
    ...
    private void init(){
        inflate(getContext(), R.layout.activity_write_html, this);
        ...
        content.setOnDecorationChangeListener(new RichEditor.OnDecorationStateListener() {
            ...
            public void onStateChangeListener(String text, List<RichEditor.Type> types) {
                Buttons = new ArrayList<>(Arrays.asList(textColorButton, textBgColorButton, textBoldButton, textItalicButton, textUnderlineButton, textStrikeButton, textNumbers, textBullets));
                for(RichEditor.Type type : types){
                    if(type.name().contains("FONT_COLOR")){
                        ...
                        switch(type){
                            case BOLD:
                                textBoldButton.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textBoldButton.getCheckedState())
                                    textBoldButton.switchCheckedState();
                                Buttons.remove(textBoldButton);
                                break;
                            case ITALIC:
                                textItalicButton.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textItalicButton.getCheckedState())
                                    textItalicButton.switchCheckedState();
                                Buttons.remove(textItalicButton);
                                break;
                            case UNDERLINE:
                                textUnderlineButton.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textUnderlineButton.getCheckedState())
                                    textUnderlineButton.switchCheckedState();
                                Buttons.remove(textUnderlineButton);
                                break;
                            case STRIKETHROUGH:
                                textStrikeButton.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textStrikeButton.getCheckedState())
                                    textStrikeButton.switchCheckedState();
                                Buttons.remove(textStrikeButton);
                                break;
                            case ORDEREDLIST:
                                textNumbers.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textNumbers.getCheckedState())
                                    textNumbers.switchCheckedState();
                                Buttons.remove(textNumbers);
                                break;
                            case UNORDEREDLIST:
                                textBullets.setColorFilter(ContextCompat.getColor(getContext().getApplicationContext(), R.color.colorPrimaryMid));
                                if(!textBullets.getCheckedState())
                                    textBullets.switchCheckedState();
                                Buttons.remove(textBullets);
                            break;
        ...
        textNumbers = findViewById(R.id.write_textNumbers);
        textNumbers.setOnClickListener(decorationButtonListener);

        textBullets = findViewById(R.id.write_textBullets);
        textBullets.setOnClickListener(decorationButtonListener);
