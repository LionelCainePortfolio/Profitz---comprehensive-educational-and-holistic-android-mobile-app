...
if (isset($_REQUEST["send_message"]))
{
    ...
    if ($message_type == "group")
    {
        ...
        while ($row = mysqli_fetch_object($result))
        {
            $sql = "INSERT INTO group_chat(group_id, send_to, is_read) VALUES('$send_to', '" . $row->user_id . "', '0')";
            mysqli_query($conn, $sql);
        }
   ...
   $date_to_check = $currentDateTime->format('Y-m-d H:i:s');
   $date_new = DateTime::createFromFormat('Y-m-d  H:i:s', $date_to_check)->format('Y-m-d H:i');
      $sql_messages_in_one_time = "SELECT * FROM messages WHERE ((send_from='$send_from' AND send_to='" . $row->user_id . "' AND message_type='private') OR (send_to='$send_from' AND send_from='" . $row->user_id . "' AND message_type='private')) AND (created LIKE  '".$date_new."%')";
    ...
    //pobierz z tabeli najnowsza z kilku = first_message


    //jesli data wiadomosci jest najstarsza ze wszystkich 
    // pobierz nastarszy i sprawdz czy jego messageid rownia sie z aktualnym, jesli tak to zwroc ze jest to pierwsza wiadomosc i pokazuj date w ccliencei
     $sql_messages_first_message = "SELECT * FROM messages WHERE ((send_from='$send_from' AND send_to='" . $row->user_id . "' AND message_type='private') OR (send_to='$send_from' AND send_from='" . $row->user_id . "' AND message_type='private')) AND (created LIKE  '".$date_new."%') ORDER BY created ASC LIMIT 1";
    ...
    //pobierz z tabeli najstarsza z kilku = last_message

    $sql_messages_last_message = "SELECT * FROM messages WHERE ((send_from='$send_from' AND send_to='" . $row->user_id . "' AND message_type='private') OR (send_to='$send_from' AND send_from='" . $row->user_id . "' AND message_type='private')) AND (created LIKE  '".$date_new."%') ORDER BY created DESC LIMIT 1";
...
if (isset($_REQUEST["get_home_metadata"])) {
    ...
    // Analogicznie dla pozostałych zapytań...

    // Close the prepared statements
    mysqli_stmt_close($stmt1);
    // Analogicznie dla pozostałych prepared statements...
