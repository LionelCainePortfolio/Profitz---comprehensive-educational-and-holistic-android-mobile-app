...
if ($featured == "yes") {
    $query_drops = mysqli_query($conn, "SELECT * FROM airdrops WHERE visibility='1' AND featured='1' ORDER BY date_add DESC");
} 

while ($row = mysqli_fetch_assoc($query_drops)){
	array_push($response,
	array('airdrop_id'=>$row['id'],
	...
	'airdrop_facebook_required'=>$row['facebook_required'],
	...
	'airdrop_date_finish'=>$row['airdrop_date_finish']
));
}
echo json_encode($response);
    ...
    $query_drops = mysqli_query($conn, "SELECT * FROM airdrops WHERE visibility='1' AND featured='0' AND $conditions_str ORDER BY date_add DESC");
}
